...
- [Specification](#specification)
  ...
  - [Window Events](#window-events)
    - [Announce and Request Events](#announce-and-request-events)
- [Backwards Compatibility](#backwards-compatibility)
- [Copyright](#copyright)

## Summary

A standard mechanism for discovering multiple injected TRON Wallet Providers in a web page using JavaScript `window` events, as an alternative to `window.tron`.

## Abstract

An alternative discovery mechanism to `window.tron` for [TIP-1193](https://github.com/tronprotocol/tips/blob/master/tip-1193.md) providers which supports discovering multiple injected Wallet Providers in a web page using JavaScript `window` events.

## Motivation

Currently, Wallets that offer browser extensions must inject their TRON providers ([TIP-1193](https://github.com/tronprotocol/tips/blob/master/tip-1193.md)) into the same window object `window.tron`; however, this creates conflicts for users that may install more than one browser extension.

Browser extensions are loaded in the web page in an unpredictable and unstable order, resulting in a race condition where the user does not have control over which Wallet Provider is selected to expose the TRON interface under the `window.tron` object. Instead, the last wallet to load usually wins.
...
Some browser extensions attempt to counteract this problem by delaying their injection to overwrite the same `window.tron` object which creates an unfair competition for Wallet Providers and lack of interoperability.
...
**Wallet Provider**: A user agent that manages keys and facilitates transactions on the TRON network.

**Decentralized Application (DApp)**: A web page that relies upon one or more Web3 APIs exposed through the wallet.

**Provider Discovery Library**: A library or piece of software that assists a DApp in interacting with the wallet.

### Provider Info

Each wallet provider must announce itself using the `TIP6963ProviderInfo` interface. All required fields defined in this interface must be included in the provider info object.

- **`uuid`** - a globally unique identifier of the wallet provider that must be ([UUIDv4][RFC-4122]-compliant) to uniquely distinguish different [TIP-1193](https://github.com/tronprotocol/tips/blob/master/tip-1193.md) provider sessions that have matching properties defined below during the lifetime of the page. The cryptographic uniqueness provided by [UUIDv4][RFC-4122] guarantees that two independent `TIP6963ProviderInfo` objects can be separately identified.
...
The `TIP6963ProviderDetail` is used as a composition interface to announce a Wallet Provider and related metadata about the Wallet Provider. The `TIP6963ProviderDetail` must contain an `info` property of type `TIP6963ProviderInfo` and a `provider` property of type `TIP1193Provider` defined by [TIP-1193](https://github.com/tronprotocol/tips/blob/master/tip-1193.md).
